...
# HTTP 헤더
# WhatIsMyBrowser 페이지에서 해당 정보 가져옴
...
# 1. url (필수): 가져올 웹 페이지의 URL을 문자열로 전달
# 2. params: URL에 추가할 패러미터를 딕셔너리 형태로 전달
# 3. headers: 웹 페이지 요청시 함께 전달될 HTTP 헤더를 딕셔너리 형태로 전달 (User-Agent 설정 시 사용)
