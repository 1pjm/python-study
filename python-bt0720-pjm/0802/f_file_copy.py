...
# : 원본 파일에서 한 번에 읽어 들이는 데이터 크기를 1KB(1024Byte)로 설정해 복사 프로그램을 구현
# 2-4. 파일 닫기 (with문 사용 시 생략 가능)
video_size = 1024 # 1024Byte = 1KB
with open('production_id_4812205 (1080p).mp4', 'rb') as source:
    with open('copy_video.mp4', 'wb') as copy:
        while True:
            buffer = source.read(video_size) # read(파일의 용량을 지정)
            if not buffer:
                break
            copy.write(buffer)
