### 반복문(Loop) ###
# 특정 코드들을 여러 번 실행하도록 프로그램에 지시하는 프로그래밍 구조
# 반복문의 종류: for, while

# 1. For 반복문 사용법
# : 시퀀스(리스트, 튜플, 문자열 등)나 다른 반복 가능한 객체를 순회하는데 사용
'''
for 변수 in 시퀀스:
    반복할 코드
...
# 시작 값, 종료 값, 스텝 값(건너뛰는 값)을 지정 가능: range(시작 값, 종료 값, 스텝 값)
# 1부터 10까지 2씩 증가하는 숫자를 출력(홀수 출력)
...
# items() 메서드는 딕셔너리의 모든 키값 쌍을 반환하는 메서드
# 키: keys()
# 값: values()
